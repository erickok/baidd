 ...
 *
 */
public class NonArguingAgent implements Agent, StrategyExposer {

	private static final String NAME = "Non arguing agent";
	...
	public NonArguingAgent() {
		...
		this.properties.put(Property.BaseAttitudeIsDestory, Boolean.TRUE);
		this.properties.put(Property.PlayBuildOrDestroyStrategy, Boolean.FALSE);
	 ...
	 */
	public NonArguingAgent(String name, Map<String, Object> rawProperties, KnowledgeBase beliefs, List<Rule> optionBeliefs, List<Goal> goalsHidden, List<Goal> goalsPublic) {
		this();
		this.name = name;
	 ...
	 */
	public NonArguingAgent(AgentXmlData xmlDataFile) {
	...
	public List<Move<? extends Locution>> makeMoves() {
		...
		try {
			...
			for (ValuedOption option : valuedOptions) {
				...
				// Play arguments supporting this proposal
				...
				if (!playBuildOrDestroyStrategy || (option.getStrategy() == Strategy.Build && existingIsIn)) {
					RuleArgument newArgue = helper.generateRebuttal(this.beliefs, dialogue.getTopicGoal().getGoalContent(), 0.0, existingProposal.getProposalMove(), existingProposal.getReplies(existingProposal.getProposalMove()), Arrays.asList(new Rule(existingProposal.getProposalLocution().getConcreteProposal())));
					// Found an argument? Then make the move
					if (newArgue != null) {
						moves.add(Move.buildMove(this.participant, existingProposal.getProposalMove(), new ArgueLocution(newArgue)));
						break;
					}
				}

				// Play arguments attacking this proposal
				if (!playBuildOrDestroyStrategy || (option.getStrategy() == Strategy.Destroy && !existingIsIn)) {
					RuleArgument newArgue = helper.generateRebuttal(this.beliefs, dialogue.getTopicGoal().getGoalContent().negation(), 0.0, existingProposal.getProposalMove(), existingProposal.getReplies(existingProposal.getProposalMove()), Arrays.asList(new Rule(existingProposal.getProposalLocution().getConcreteProposal())));
					// Found an argument? Then make the move
					if (newArgue != null) {
						moves.add(Move.buildMove(this.participant, existingProposal.getProposalMove(), new ArgueLocution(newArgue)));
						break;
	...
	private void analyseOptions(List<ValuedOption> valuedOptions) {
		...
		for (ValuedOption option : valuedOptions) {
			...
			if (!(Boolean)this.properties.get(Property.BaseAttitudeIsDestory)) {
